...
public class MZClient
{
    ...
    {
        var match = Regex.Match (serviceDescription, @"(_[a-z]+._tcp|udp)\s*(\d+)\s*(.*)") ;
        ...
        if (txt_data != null)
        {
            var tmatch = Regex.Match (txt_data, @"TXT\s*\[(.*)\]") ;
            ...
            foreach (var part in Regex.Split (txt_data, @"'\s*,"))
            {
                var expr = part.Trim () ;
                if (!expr.EndsWith ("'"))
                    expr += "'" ;

                var pmatch = Regex.Match (expr, @"(\w+\s*\w*)\s*=\s*['](.*)[']\s*") ;
                var key    = pmatch.Groups[1].Value.Trim () ;
                var val    = pmatch.Groups[2].Value.Trim () ;
    ...
    private static void OnServiceResolved (object o, ServiceResolvedEventArgs args)
    {
        ...
        var record_count = record != null ? record.Count : 0 ;
        if (record_count > 0)
        {
