...
public sealed class BrowseService : IResolvableService
{
    ...
    public void Resolve ()
    {
        this.ResolveAsync ().ConfigureAwait (false).GetAwaiter ().GetResult () ;
    }
    ...
    public async Task ResolveAsync (CancellationToken cancellationToken = default)
    {
        ...
        var result = await Task.Run (() => MdnsClient.Resolve (new MdnsClient.MdnsServiceInfo
                                                                {
                                                                    Name = this.Name,
        ...
        this.TxtRecord = ArkaneSystems.Arkane.Zeroconf.Providers.WindowsMdns.TxtRecord.FromEntries (result.TxtEntries) ;
